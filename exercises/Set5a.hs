...
data Vehicles = Bike | Bus | Tram | Train
  deriving Show
...
totalPrice (MkShoppingEntry _ itemPrice count) = itemPrice * fromIntegral count
...
buyOneMore (MkShoppingEntry itemName itemPrice count) = MkShoppingEntry itemName itemPrice (count+1) 
...
setName newName (MkPerson _ age) = MkPerson newName age

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge newAge (MkPerson name _) = MkPerson name newAge
...
data UpDown = MkUpDown Int Int

instance Show UpDown where
  show (MkUpDown dir i) = let dirName = if dir == 1 then "Up" else "Down"
                          in "(" ++ dirName ++ ", " ++ show i ++ ")"
...
tick :: UpDown -> UpDown
tick (MkUpDown dir c) = MkUpDown dir (c+dir)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (MkUpDown dir c) = MkUpDown (-1*dir) c
...
rgb (Mix c1 c2) = let ave e1 e2 = ((e1+e2)/2)
                  in zipWith ave (rgb c1) (rgb c2)
...
toList Empty        = []
toList (Pair k v t)   = (k,v) : toList t
...
toNat n
  | n < 0     = Nothing
  | otherwise = Just (toNat_ n)
  where toNat_ 0 = Zero
        toNat_ n = PlusOne (toNat_ (n-1))
...
toBin 0 = O End
toBin 1 = I End
toBin q = selectIO (q `mod` 2) (toBin (q `div` 2))
          where 
            selectIO 0 = O
            selectIO 1 = I

-- A solution from : https://www.splashlearn.com/math-vocabulary/decimal-to-binary#:~:text=To%20convert%20decimal%20numbers%20to,0%20as%20the%20final%20quotient.
-- 1. Divide n by 2 remember remainder and quotient
-- 2. device quotient by 2, and not r & q
-- 3. Repeat above, until you get q = 0
-- 4. Write the remainders in reverse order from last to first
-- 5. LSB is at top and MSB is at bottom
